...
export default {
  async fetch(request, env) {
    ...
    // ======================================================
    // UPLOAD FILE (MANUAL FETCH — WORKS IN CLOUDFLARE)
    // ======================================================
    if (pathname.startsWith("/stores/") && pathname.endsWith("/upload") && method === "POST") {
      ...
      for (const file of files) {
        ...
        console.log("UPLOAD FILE:", cleanedName, "MIME:", mimeType);
        ...
        const uploadUrl =
          `https://generativelanguage.googleapis.com/v1beta/${fsStoreName}:uploadToFileSearchStore?key=${apiKey}`;
        ...
        try {
          ...
          results.push({
            ...
            uploaded: false,
            indexed: false,
            gemini_error: "Network error: " + netErr.toString()
          });
          continue;
        }
        ...
        if (!uploadResp.ok) {
          results.push({
            ...
            uploaded: false,
            indexed: false,
            gemini_error: await uploadResp.text()
          });
          continue;
        }
        ...
        try {
          ...
          results.push({
            ...
            uploaded: false,
            indexed: false,
            gemini_error: "Invalid JSON from Gemini: " + parseErr.toString()
          });
          continue;
        }

        const docResource = uploadJson?.fileSearchDocument?.name || null;
    ...
    if (pathname === "/stores" && method === "GET") {
      ...
      try {
        ...
        return json({ error: err.toString() }, 400);
      }
    ...
    if (pathname === "/ask" && method === "POST") {
      ...
      if (fsStores.length === 0) return json({ error: "No valid stores found" }, 400);
      ...
      try {
        const resp = await ai.models.generateContent({
          ...
          config: {
            systemInstruction:
              systemPrompt || "Answer ONLY based on the stored documents.",
            tools: [
              {
                fileSearch: {
        ...
        return json({
          ...
          grounding_metadata: resp.candidates?.[0]?.groundingMetadata || null
        });
      } catch (err) {
        return json({ error: err.toString() }, 500);
      }
